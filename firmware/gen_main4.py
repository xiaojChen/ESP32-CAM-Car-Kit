code = r'''/* ESP32-CAM Car v4 - Landscape + PWM
...
.col-l{flex:0 0 90px;height:100%;justify-content:center}
.col-r{flex:0 0 90px;height:100%;justify-content:center}
.vid{flex:1;display:flex;align-items:center;justify-content:center;height:100%;overflow:hidden}
.vid img{width:100%;height:100%;object-fit:contain;border-radius:8px;background:#000}
button{width:70px;height:70px;border:none;border-radius:50%;font-size:24px;color:#fff;cursor:pointer;-webkit-tap-highlight-color:transparent;touch-action:manipulation;user-select:none;display:flex;align-items:center;justify-content:center}
.f{background:#e94560}
.b{background:#0f3460}
.l{background:#16213e}
.r{background:#16213e}
...
rf();
document.getElementById("info").textContent=location.hostname;
</script>
</body></html>
)rawliteral";
...
void setup(){
  ...
  IPAddress ip=WiFi.softAPIP();Serial.print("AP IP: ");Serial.println(ip);
  // Routes
...
'''
import os; os.makedirs('D:/esp32-cam-car-pio/src',exist_ok=True)
with open('D:/esp32-cam-car-pio/src/main.cpp','w',encoding='utf-8') as f:f.write(code)
print('Written, size:',len(code))
